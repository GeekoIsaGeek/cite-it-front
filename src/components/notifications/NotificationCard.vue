...
<template>
  ...
  >
    <div class="flex items-center justify-between px-6 py-6 gap-6 w-full">
      <div class="flex items-center gap-6">
        <img
          :src="avatarPlaceholder"
          alt="user avatar"
          :class="`rounded-full w-20 h-20 ${isNew && 'border-2 border-[#198754]'}`"
        />
        <div class="flex flex-col gap-2">
          <p text-white>Nino Tabagari</p>
          <div class="flex items-center gap-3" v-if="hasCommented">
      ...
      <div :class="`flex flex-col self-${isNew ? 'center' : 'start'}`">
